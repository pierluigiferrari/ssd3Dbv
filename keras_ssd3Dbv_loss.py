'''The Keras-compatible loss function for the SSD3DBV model. Currently supports TensorFlow only.'''
...
class SSD3DBV_Loss:
    ...
    def smooth_L1_loss(self, y_true, y_pred):
        ...
        Arguments:
            y_true (nD tensor): A TensorFlow tensor of any shape containing the ground truth data.
                In this context, the expected tensor has shape `(batch_size, #boxes, k)` and
                contains the ground truth bounding box coordinates, where the last dimension
                contains `k` box coordinates.
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        Arguments:
            y_true (array): A Numpy array of shape `(batch_size, #boxes, #classes + 18)`,
                where `#boxes` is the total number of boxes that the model predicts
                per image. Be careful to make sure that the index of each given
                box in `y_true` is the same as the index for the corresponding
                box in `y_pred`. The last axis must have length `#classes + 18` and contain
                `[classes one-hot encoded, 9 ground truth box coordinates, 9 arbitrary entries]`
                in this order, including the background class. The last 9 entries of the
                ...
                where the last 9 entries of the last axis contain the anchor box
        ...
        localization_loss = tf.to_float(self.smooth_L1_loss(y_true[:,:,-18:-9], y_pred[:,:,-18:-9])) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets

        # Create masks for the positive and negative ground truth classes
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.to_float(tf.reduce_max(y_true[:,:,1:-18], axis=-1)) # Tensor of shape (batch_size, n_boxes)
